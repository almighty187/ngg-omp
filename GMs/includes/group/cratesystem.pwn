...
{
	if (PlayerBusy(playerid))
		return SendClientMessageEx(playerid, COLOR_GRAD2, "You can't do this right now");

	new string[128];
	new count = 0;

	SendClientMessageEx(playerid, COLOR_YELLOW, "----- Crate Facilities -----");
	...
	for (new f = 0; f < MAX_CRATE_FACILITY; f++)
	{
		if (CrateFacility[f][cfPos][0] != 0.0)
		{
			...
			{
				format(string, sizeof(string), "Facility: %s | Cooldown: %s",
					CrateFacility[f][cfName],
					ConvertTimeS(CrateFacility[f][cfCooldown] - gettime()));
			}
			else
			{
				format(string, sizeof(string), "Facility: %s | Cooldown: None",
					CrateFacility[f][cfName]);
			}
	...
	if (!count)
		SendClientMessageEx(playerid, COLOR_GRAD2, "No crate facilities found.");
